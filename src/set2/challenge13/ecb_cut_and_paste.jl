...
export Server, is_admin, profile_for, encrypt_user_profile, decrypt_user_profile, lift_to_admin
...
struct Server
    ...
    Server() = new(rand(UInt8, 16))  # a very random AES key
end
...
"""
function encrypt_user_profile(svr::Server, user_profile::String)::Vector{UInt8}
    bytes = Vector{UInt8}(user_profile)
    pkcs7_padding!(bytes, 16)
    aes_128_ecb_encode(bytes, svr.key)
end

"The reverse of `encrypt_user_profile`."
function decrypt_user_profile(svr::Server, user_profile::Vector{UInt8})::String
    String(trim_padding_PKCS5(aes_128_ecb_decode(user_profile, svr.key)))
end

function encrypted_profile_for(svr::Server, email::String)::Vector{UInt8}
    user_profile = profile_for(email)
    encrypt_user_profile(svr, user_profile)
end

#===== Cracker =====#
function lift_to_admin(enc_user_profile::Vector{UInt8}, profile_oracle::Server)::Vector{UInt8}
